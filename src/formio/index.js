...
export default class Formio {
    ...
    return fetch(url, options)
      .then((response) => {
        ...
        if (
          ...
          console.warn('Token was introduced in request.'); //eslint-disable-line no-console
  ...
  static async setToken(token, opts) {
    ...
    const tokenName = `${opts.namespace || 'formio'}Token`;
    if (!Formio.tokens) {
      Formio.tokens = {};
    }
  ...
  static async getToken(options) {
    ...
    const tokenName = `${options.namespace || 'formio'}Token`;
    if (!Formio.tokens) {
      Formio.tokens = {};
    }
    ...
    Formio.tokens[tokenName] = await AsyncStorage.getItem(tokenName) || '';
    return Formio.tokens[tokenName];
  }

  static async setUser(user, opts) {
    opts = opts || {};
    const userName = `${opts.namespace || 'formio'}User`;
    if (!user) {
      this.setToken(null, opts);
      return AsyncStorage.removeItem(userName);
    }
    return await AsyncStorage.setItem(userName, JSON.stringify(user));
  }

  static async getUser(options) {
    options = options || {};
    const userName = `${options.namespace || 'formio'}User`;
    const user = await AsyncStorage.getItem(userName) || null;
  ...
  static setAppUrl(url) {
    console.warn('Formio.setAppUrl() is deprecated. Use Formio.setProjectUrl instead.'); //eslint-disable-line no-console
    Formio.projectUrl = url;
  ...
  static getAppUrl() {
    console.warn('Formio.getAppUrl() is deprecated. Use Formio.getProjectUrl instead.'); //eslint-disable-line no-console
